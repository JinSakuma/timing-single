...
def run(config_path, gpu_device=-1):
	...
	model.dialog_acts_model.encoder.load_state_dict(torch.load(config.da_continue_from_checkpoint), strict=False)
	model.system_acts_model.encoder.load_state_dict(torch.load(config.sa_continue_from_checkpoint), strict=False)
	model.to(device)
