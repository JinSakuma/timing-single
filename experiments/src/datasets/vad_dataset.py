...
def collate_fn(batch):
    ...
    indices, uttr_type, cnnae, fbank, input_lengths, labels, label_lengths, uttr_labels = zip(*batch)
    ...
    uttr_labels_ = torch.zeros(batch_size, wav_len).long()

    for i in range(batch_size):
        l1 = len(cnnae[i])
        l2 = len(fbank[i])
        l3 = len(uttr_labels[i])
        cnnae_[i, :l1, :] = cnnae[i]
        fbank_[i, :l2, :] = fbank[i]
        ...
        uttr_labels_[i, :l3] = torch.tensor(uttr_labels[i][:wav_len]).long()

    return uttr_nums, uttr_type_, cnnae_, fbank_, input_lengths_, labels_, label_lengths_, uttr_labels_
