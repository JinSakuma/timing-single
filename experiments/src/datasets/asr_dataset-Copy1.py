...
def collate_fn(batch):
    ...
    indices, uttr_type, inputs, input_lengths, labels, label_lengths = zip(*batch)
    
    batch_size = len(indices)
    
    uttr_nums = torch.tensor([len(l) for l in labels])
    
    wav_len = max([len(inp) for inp in inputs])
    label_len = max([max(l) for l in label_lengths])
    
    dim = inputs[0].shape[-1]
    
    uttr_type_ = torch.zeros(batch_size).long()
    inputs_ = torch.zeros(batch_size, wav_len, dim)
    ...
    for i in range(batch_size):
        l = len(inputs[i])
        inputs_[i, :l, :] = inputs[i]
            
        uttr_type_[i] = torch.tensor(uttr_type[i]).long()       
        input_lengths_[i] = torch.tensor(input_lengths[i]).long()
        labels_[i, :] = torch.tensor(np.array(labels[i])[:, :label_len]).long()
        label_lengths_[i] = torch.tensor(label_lengths[i]).long()

    return uttr_nums, uttr_type_, inputs_, input_lengths_, labels_, label_lengths_
