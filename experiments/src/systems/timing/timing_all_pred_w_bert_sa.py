...
class System(nn.Module):
	...
	def create_models(self):
		...
		timing_model = RTG(
			self.device,
			self.config.model_params.input_dim+self.config.model_params.cnnae_dim+self.system_acts_num_class+self.text_encoding_dim+1,
		...
		if self.config.loss_params.timing_weight>0:
			...
			for j in range(uttr_nums[0]):
				...
				if dur-self.T>0 and embedding[i].size(0)-(dur-self.T)>0:
					...
					tmp = torch.zeros([(dur-self.T), self.system_acts_num_class+self.encoding_dim]).to(self.device)
					...
					dasa = torch.cat([sa, text_emb], dim=-1)
