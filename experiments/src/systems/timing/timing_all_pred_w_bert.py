...
from src.systems.slu.slu_bert3 import SLU
...
class System(nn.Module):
	...
	def create_models(self):
		...
		timing_model = RTG(
			self.device,
# 			self.config.model_params.input_dim+self.config.model_params.cnnae_dim+128+128+self.text_encoding_dim+1,
			self.config.model_params.input_dim+self.config.model_params.cnnae_dim+16+17+self.text_encoding_dim+1,
		...
		if self.config.loss_params.timing_weight>0:
			...
			for j in range(uttr_nums[0]):
				...
				if dur-self.T>0 and embedding[i].size(0)-(dur-self.T)>0:
					...
					tmp = torch.zeros([(dur-self.T), 128+128+self.encoding_dim]).to(self.device)
