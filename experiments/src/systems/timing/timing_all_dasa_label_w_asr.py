...
class System(nn.Module):
	...
	def create_models(self):
		...
		timing_model = RTG(
			self.device,
			self.config.model_params.asr_hidden_dim+self.config.model_params.cnnae_dim+self.dialog_acts_num_class+self.system_acts_num_class+1,
	...
	def forward(self, batch, split='train'):
		...
		#dialog_acts_probs = outputs['dialog_acts_probs']
		#system_acts_probs = outputs['system_acts_probs']

		i=0
		asr_inputs = torch.nn.functional.avg_pool1d(fbank[i].transpose(1, 2), kernel_size=5, ceil_mode=True).transpose(1, 2)
		log_probs, _, asr_embedding = self.asr_model(asr_inputs, input_lengths[i])
		length = max(input_lengths[i])
		embedding = torch.cat([asr_embedding[:, :length, :], cnnae[i][:, :length, :]], dim=-1)	
		...
		if self.config.loss_params.timing_weight>0:
			...
			for j in range(uttr_nums[i]):
				...
				if dur-self.T>0 and embedding[j].size(0)-(dur-self.T)>0:
					t = embedding[j].size(0)-(dur-self.T)
					tmp = torch.zeros([(dur-self.T), self.dialog_acts_num_class+self.system_acts_num_class]).to(self.device)
