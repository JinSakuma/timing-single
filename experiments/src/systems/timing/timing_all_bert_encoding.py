...
from src.systems.text_encoder.bert_encoder import TextEncoder
...
class System(nn.Module):
	...
	def create_models(self):
		...
		self.text_encoding_dim = text_encoder.encoding_dim

		timing_model = RTG(
			self.device,
			self.config.model_params.input_dim+self.config.model_params.cnnae_dim+self.text_encoding_dim+1,
	...
	def forward(self, batch, split='train'):
		...
		outputs = self.text_encoder.recog(batch)
		...
		if self.config.loss_params.timing_weight>0:
			...
			for j in range(uttr_nums[0]):
				...
				if dur-self.T>0 and embedding[i].size(0)-(dur-self.T)>0:
					...
					tmp = torch.zeros([(dur-self.T), self.text_encoding_dim]).to(self.device)
					text_emb = text_encoding[j].unsqueeze(0).repeat(t, 1)
					tmp2 = torch.cat([tmp, text_emb], dim=0)
				else:
					tmp2 = torch.zeros([embedding[j].size(0), self.text_encoding_dim]).to(self.device)
