...
from src.systems.slu.slu_bert import SLU
...
class System(nn.Module):
	...
	def __init__(self, config, device, asr_input_dim, asr_num_class, dialog_acts_num_class, system_acts_num_class):
		...
		self.encoding_dim = config.model_params.text_hidden_dim
		self.create_models()
	...
	def create_models(self):
		...
		self.text_encoding_dim = self.encoding_dim
        
		self.subsampling = Conv2dSubsampling5(self.asr_input_dim)        

		timing_model = TGN(
			self.device,
			self.config.model_params.input_dim+self.config.model_params.cnnae_dim+self.dialog_acts_num_class+self.system_acts_num_class+self.text_encoding_dim+1,
	...
	def forward(self, batch, split='train'):
		...
		if self.config.loss_params.timing_weight>0:
			...
			for j in range(uttr_nums[0]):
				...
				ll, _ = self.timing_model(emb, uttr_labels[i][j][:input_lengths[i][j]], timings[i][j][:input_lengths[i][j]], uttr_labels[i][j][:input_lengths[i][j]])
				#timing_loss = timing_loss+self.timing_model.get_loss(output, timings[0][i][:input_lengths[i]])
				timing_loss = timing_loss+ll
		loss = (
