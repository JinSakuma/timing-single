...
from src.models.encoders.bert_encoder2 import BertEncoder

from src.models.asr.hubert.hubert3 import CTC
from src.models.dialog_acts.linear import (
    SystemActsPredictor,
    DialogActsPredictor,
)
...
class SLU(nn.Module):
    ...
    def create_models(self):
        ...
        self.fc = nn.Linear(self.bert_hidden_dim, self.encoding_dim)
...
#                 total += 1
        ...
        # context encoding
        emb = self.bert_encoder(bert_labels, bert_masks, uttr_nums)
        emb = self.fc(emb)
        #print(emb.shape)
        # b, d, h = emb.shape
        # emb = pooled_out_.view(b*d, -1)
