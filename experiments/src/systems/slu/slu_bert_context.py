...
class SLU(nn.Module):
    ...
    def create_models(self):
        ...
        dialog_acts_model = DialogActsPredictor(
            self.embedding_dim+self.bert_hidden_dim,
...
#         for i in range(batch_size):
        ...
        if self.config.loss_params.system_acts_weight>0:
            ...
            system_acts_loss = self.system_acts_model.get_loss(system_acts_probs, system_acts_labels[0])
            system_acts_p, system_acts_r, system_acts_f1, system_acts_acc = self.f1_score_dialog_acts(system_acts_probs, system_acts_labels[0])
