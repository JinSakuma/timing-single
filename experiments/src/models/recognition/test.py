...
from src.models.recognition.asr2 import CTC
...
class E2ESLU(nn.Module):
	...
	def __init__(self, config, device, asr_input_dim, asr_phoneme_num, asr_word_num, dialog_acts_num_class):
		super().__init__()
		self.config = config
		self.device = device
		self.asr_input_dim = asr_input_dim
		self.asr_phoneme_num = asr_phoneme_num
		self.asr_word_num = asr_word_num
	...
	def asr_pretrain(self, batch, split='train'):
		inputs = batch[0].to(self.device)
		input_lengths = batch[1].to(self.device)
		phon_labels = batch[2].to(self.device)
		phon_lengths = batch[3].to(self.device)
		word_labels = batch[4].to(self.device)
		word_lengths = batch[5].to(self.device)

		log_probs_p, log_probs_w = self.asr_model.forward_pretrain(inputs, input_lengths)

		asr_loss_p = self.get_asr_loss(log_probs_p, input_lengths, phon_labels, phon_lengths)
		asr_loss_w = self.get_asr_loss(log_probs_w, input_lengths, word_labels, word_lengths)

		asr_loss = asr_loss_p + asr_loss_w

		if split=="test":
			cer = self.get_wer(log_probs_p, input_lengths, phon_labels, phon_lengths)
			wer = self.get_wer(log_probs_w, input_lengths, word_labels, word_lengths)
			return asr_loss, cer, wer
