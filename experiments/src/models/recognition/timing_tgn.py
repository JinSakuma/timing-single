...
from src.models.timing.tgn import TGN
from src.models.recognition.context_encoder import ContextEncoder
from src.models.recognition.tasks3 import (
    SystemActsPredictor,
    DialogActsPredictor,
)
...
class E2ESLU(nn.Module):
	...
	def create_models(self):
		...
		timing_model = TGN(
			self.device,
			self.config.model_params.asr_hidden_dim+self.config.data_params.n_mels,
	...
	@torch.no_grad()
	def get_vad(self, inputs):
		return  self.vad(inputs)
		...
		if self.config.loss_params.timing_weight>0:
			...
			for i in range(uttr_nums[0]):
				if uttr_type[0][i] == 0 or uttr_type[0][i] == 1:
					...
					emb = embedding[i][:input_lengths[i]]
					timing = timings[i][:input_lengths[i]]
					timing_label = timing[1:]-timing[:-1]

					uttr_prob = self.get_vad(emb)
					uttr_prob = torch.ones(uttr_prob.shape).to(self.device) - uttr_prob

					l, _ = self.timing_model(emb, uttr_labels[i][:input_lengths[i]], timing_label.detach(), uttr_labels[i][:input_lengths[i]])
					#l, _ = self.timing_model(emb, uttr_prob, timing_label.detach(), uttr_labels[i][:input_lengths[i]])
					if uttr_type[0][i]==0:
						timing_loss = timing_loss + l
