...
from src.models.timing.satg import SATG
from src.models.recognition.context_encoder2 import ContextEncoder2
from src.models.recognition.tasks_ import (
    SystemActsPredictor,
    DialogActsPredictor,
)
...
class E2ESLU(nn.Module):
	...
	def create_models(self):
		...
		timing_model = SATG(
			...
			self.config.model_params.asr_hidden_dim+self.config.data_params.n_mels+self.dialog_acts_num_class*2,
	...
	def forward(self, batch, split='train'):
		...
		if self.config.loss_params.dialog_acts_weight>-1:
			dialog_acts_probs = self.dialog_acts_model(emb)
			...
			#print(dialog_acts_p, dialog_acts_r, uttr_type[0])
		else:
			dialog_acts_loss = 0
		...
		if self.config.loss_params.system_acts_weight>-1:
			...
			system_acts_probs, system_acts_labels = self.system_acts_model(emb, system_acts_labels, uttr_type[0], uttr_nums)
		...
		if self.config.loss_params.timing_weight>0:
			...
			for i in range(uttr_nums[0]):
				if uttr_type[0][i] == 0 or uttr_type[0][i] == 1:
					if offset[0][i]>3000:
						continue
					context = torch.cat([dialog_acts_probs[i], system_acts_probs[i]], dim=-1)
					t = embedding[i].size(0)
					context = context.unsqueeze(0).repeat(t,1)
	...
	def f1_score_dialog_acts(self, outputs, labels):
		...
		P /= float(outputs.shape[0])
		R /= float(outputs.shape[0])
		F1 /= float(outputs.shape[0])
		return P, R, F1, acc	
