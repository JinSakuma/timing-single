...
class E2ESLU(nn.Module):
	...
	def create_models(self):
		...
		timing_model = RTG(
			self.device,
			self.config.model_params.timing_hidden_dim+self.config.data_params.n_mels,
	...
	def configure_optimizer_parameters(self):
		...
		elif  self.config.loss_params.vad_weight==1.0:
			parameters = chain(
				self.vad.parameters(),
			)
	...
	def forward(self, batch, split='train'):
		...
		input_lengths = batch[2]#.to(self.device)
		timings = batch[3].to(self.device)
		uttr_labels = batch[4].to(self.device)
		labels = batch[5].to(self.device)
		label_lengths = batch[6]#.to(self.device)
		dialog_acts_labels = batch[7].to(self.device)
		system_acts_labels = batch[8].to(self.device)
		...
		if  self.config.loss_params.vad_weight>0:
			assert batch_size==1, "batch size must be set 1"
			for i in range(uttr_nums[0]):
				output = self.vad(embedding[i][:input_lengths[i]])
				vad_loss = vad_loss+self.vad.get_loss(output, uttr_labels[i][:input_lengths[i]])	
				vad_acc = vad_acc+self.vad.get_acc(output, uttr_labels[i][:input_lengths[i]])	
			vad_acc = vad_acc / float(uttr_nums[0])
		...
		if  self.config.loss_params.timing_weight>0:
			assert batch_size==1, "batch size must be set 1"
			for i in range(uttr_nums[0]):
				output = self.timing_model(embedding[i][:input_lengths[i]], uttr_labels[i][:input_lengths[i]])
		...
		loss = (
			...
			vad_loss * self.config.loss_params.vad_weight +
