...
from s3prl.hub import hubert #, distilhubert
...
class CTC(nn.Module):
    ...
    def forward(self, wavs):
        ...
        #input_lengths = results['pad_mask'].sum(dim=-1).int()
        
        representation = results["last_hidden_state"]
        max_wav = representation.size(1)
        input_lengths = [min(max_wav, len(wav)// (16000*0.02)) for wav in wavs]
        input_lengths = torch.tensor(input_lengths).int().to(self.device)    
